...
因为 Dragonfly 支持多种会话类型，但能力并不完全相同：
...
切换到串口标签并重新展开端口下拉时，Dragonfly 会重新加载可用串口列表。
...
2. Dragonfly 下载该文件到本地临时目录并启动监听
3. 你在本地编辑器里保存这个文件

如果你是自己另外复制了一个文件再编辑，Dragonfly 不会知道它和远端文件之间的关系。
...
在手动处理前，建议先备份 `~/.dragonfly/` 目录，再决定如何重建本地配置。
...
### Dragonfly 的配置文件存放在哪里？

当前主要用户数据保存在 `~/.dragonfly/dragonfly.redb` 中，其中会保存设置、连接、密钥、密码、OTP、隧道、代理、历史、AI 历史等本地数据。

旧版本的 JSON / 文本文件会在首次启动时迁入 redb；迁移完成后，redb 才是主要存储位置。
