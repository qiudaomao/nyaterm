...
Dragonfly 会在本地保存连接相关配置，并对敏感内容做加密处理后再写入本地存储。常见的敏感数据包括：
...
Dragonfly 当前把主要用户数据保存在 `~/.dragonfly/dragonfly.redb` 中，而不是继续以多个 JSON 文件作为主存储。
...
旧版本的 `.dragonfly` JSON / 文本文件会在首次启动时迁入 redb，迁移完成后不会继续作为新的主写入路径。
