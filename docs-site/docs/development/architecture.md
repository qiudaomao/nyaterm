...
Dragonfly 是一个基于 **Tauri 2** 的桌面应用：前端在 `src/`，后端在 `src-tauri/src/`，两者通过 Tauri command 与事件通信。
...
Dragonfly 的终端工作区有两个容易混淆、但职责不同的层次：
...
旧版 `.dragonfly` JSON / 文本文件会在首次启动时迁入 redb；迁移完成后，redb 成为主要存储位置。
